...
??? 参考代码
    ...
    ```
然而，光有指针肯定是不够的，实际上想访问的值可能需要自己去找，并且有时候根本不存在!mask[找的是滚木]，因此还需要一个辅助函数，用于查找某一值，并将最后访问到的节点提至根节点。
??? 参考代码
    ```cpp
...
### 拓展操作
对于查排名、查前驱后继，查之前先进行 access 操作即可；对于查 $k$ 大值，则是查之后进行 access 操作。!mask[然而在洛谷P3369中，本人一开始忘记在查 $k$ 大值的时候进行 access 了，但还是过了]
